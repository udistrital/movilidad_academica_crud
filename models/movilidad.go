...
type Movilidad struct {
	...
	Presupuesto     float64        `orm:"column(presupuesto)"`
	FechaInicio     time.Time      `orm:"column(fecha_inicio);type(date)"`
	FechaFinal      time.Time      `orm:"column(fecha_final);type(date)"`
	Persona         int            `orm:"column(persona)"`
	Convenio        int            `orm:"column(convenio)"`
	IdTipoMovilidad *TipoMovilidad `orm:"column(id_tipo_movilidad);rel(fk)"`
...
func GetAllMovilidad(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Movilidad)).RelatedSel()
	// query k=v
	for k, v := range query {
